...
% Ask Harb what CrystalSurfaceVectors is. Also, check if CrystalNormal ==
% Normal for non-Graphite material.
...
Result.ReflectionNormal = ReciprocalLatticeVectorsSum/sqrt(ReciprocalLatticeVectorsSum*ReciprocalLatticeVectorsSum'); %What is this comma here?
Result.CrystalNormal = CrystalSurfaceVectors/sqrt(CrystalSurfaceVectors*CrystalSurfaceVectors');
%if Result.ReflectionNormal == Result.CrystalNormal
%    disp('Hello')
%    Result
%end

%Look up the equation for this.
...
for n=1:NumberOfAtoms
    if size(Lattice.Z,2)>1
        f = Result.ScatteringFactor(Lattice.AtomicTypes(n)); %Is this rewriting something already done?
    end
